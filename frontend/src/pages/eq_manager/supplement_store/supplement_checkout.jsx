...
const stripePromise = loadStripe(import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY2);

export default function SupplementCheckout() {
  const navigate = useNavigate();
  const { state } = useLocation();
  const { items: cart, clearCart } = useCart();
  const [cardData, setCardData] = useState(state?.cards || null);
  const subtotal = useMemo(() => cart.reduce((sum, i) => sum + i.price * i.qty, 0), [cart]);

  // Card form states
  const [cardNumber, setCardNumber] = useState(cardData?.card_number || "");
  const [nameOnCard, setNameOnCard] = useState(cardData?.card_name || "");
  const [expiry, setExpiry] = useState(cardData?.expiry_date || "");
  ...
  const maxPointsAllowedByPrice = Math.floor((subtotal * MAX_DISCOUNT_RATIO) / POINT_VALUE_LKR);
  ...
  // Handle card selection from saved cards page
  useEffect(() => {
    if (state?.cards) {
      setCardData(state.cards);
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      <div className="mx-auto max-w-6xl px-4 pt-8 flex items-center">
        ...
        <h1 className="text-2xl font-semibold ml-10">Supplement Checkout</h1>
      </div>
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-8 px-4 py-10 md:grid-cols-2">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="mb-6 flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                You’re checking out {cart.length} item(s)
              </p>
            </div>
            <div className="rounded-lg bg-gray-100 px-4 py-2 text-right">
              <div className="text-xs text-gray-500">Subtotal LKR</div>
              <div className="text-lg font-semibold">{formatLKR(subtotal)}</div>
            </div>
          </div>
          {/* Use Points block */}
          <div className="mb-6 rounded-xl border border-gray-200 bg-white p-4">
            <div className="flex items-center justify-between">
              <div className="flex flex-col gap-1">
                ...
                <div className="text-xs text-gray-500">
                  Rate: 1 pt = LKR {POINT_VALUE_LKR} • Max this order: <span className="font-medium">{maxPoints}</span> pts ({Math.round(MAX_DISCOUNT_RATIO * 100)}% cap)
                </div>
              </div>
            ...
            <div className={`mt-4 grid gap-3 transition ${usePoints && maxPoints > 0 ? "opacity-100" : "opacity-40 pointer-events-none"}`}>
              <div className="flex items-center gap-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  onClick={() => navigate("/supplement/savedCards", { state: { cart, finalAmount, pointsToUse, usePoints } })}
                >
                  View saved cards
                </button>
                <button
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4">
              ...
              <ul className="text-xs text-gray-600 space-y-1">
                {cart.map((item) => (
                  <li key={item.id}>
                    {item.name} × {item.qty} ({formatLKR(item.price * item.qty)})
                  </li>
                ))}
              </ul>
            </div>
